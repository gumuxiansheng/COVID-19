...
namespace covid19
{
    ...
    void OutPutPotentialDepots(DataModel data, std::string data_folder, std::string file_name)
    {
    ...
    void VrpCapacity(const std::string &data_folder, const std::string &file_name, const int round, std::string type)
    {
        ...
        for (size_t i = 0; i < INNER_ROUND; i++)
        {
            ...
            round_costs.push_back(covid19::CalcDistanceMinMaxCost(solution, data.distance_matrix, data.depot));
    ...
    void VrpCapacityWithFolderInitialPotential(const std::string &type, const std::string &data_folder, const int round)
    {
        ...
        for (auto &&file_name : files)
        {
            VrpCapacityInitialPotential(data_folder, file_name, round, type);
        }
    }
} // namespace covid19
